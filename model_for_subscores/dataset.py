...
class divingDataset(Dataset):
    def __init__(self, data_folder, data_file, label_file, diff_file,exe_file,encoder_file, range_file, transform,
        ...
        if self.test==2:
            ...
            if not self.test:
                for i in range(len(self.tcn_range)):
                    ...
                    encs.append(self.enc[0][self.video_name[index][0] - 1].astype(np.float32))
            ...
            if self.test == 1:
                for i in range(len(self.tcn_range)):
                    ...
                    exes.append(self.exe[0][self.video_name[index][0] - 1].astype(np.float32))
                    encs.append(self.enc[0][self.video_name[index][0] - 1].astype(np.float32))
            return video_tensor, labels, diffs, exes, encs, video_name
            # return video_tensor, labels, diffs, exes, encs
