...
class divingDataset(Dataset):
    def __init__(self, data_folder, data_file, label_file, diff_file,exe_file,score_file, range_file, transform,
        ...
        self.exe = np.load(exe_file)
        self.score = np.load(score_file,allow_pickle=True)
        self.random = random
        self.test = test
        ...
        if self.test==2:
            ...
            if not self.test:
                for i in range(len(self.tcn_range)):
                    ...
                    #     if self.label[index][0] == self.video_name[index][0]:
                    # labels.append(self.label[0][self.video_name[index][0] - 1].astype(np.float32))
                    labels.append(self.label[[self.video_name[index][0]-1][0]][1:6].astype(np.float32))

                    diffs.append(self.diff[0][self.video_name[index][0] - 1].astype(np.float32))
                    exes.append(self.exe[0][self.video_name[index][0] - 1].astype(np.float32))
                    scores.append(self.score[0][self.video_name[index][0] - 1].astype(np.float32))
            if self.test == 1:
                for i in range(len(self.tcn_range)):
                    tcn = self.tcn_range[i]
                    vid_range = self.time_range[v_id]
            ...
            return video_tensor, labels, diffs, exes, scores, video_name
            # return video_tensor, labels, diffs, exes, scores
